...
export function useGaugeHistory(gaugeId: string) {
    ...
    try {
      ...
      if (stored) {
        ...
        const filtered = parsed.filter(entry => 
          entry.timestamp > cutoffTime && entry.gaugeId === gaugeId
        );
        setHistory(filtered);
      }
    } catch (error) {
  ...
  const addReading = (reading: GaugeReading) => {
    ...
    setHistory(prev => {
      ...
      if (recentEntry && (now - recentEntry.timestamp) < (MIN_TREND_INTERVAL_MINUTES * 60 * 1000)) {
  ...
  const getReadingAge = (reading: GaugeReading): number => {
    const entry = history.find(h => 
      h.gaugeId === gaugeId && 
      h.reading.timestamp === reading.timestamp
    );
