...
│   └── Footer.tsx   # App footer with copyright, clickable "Ozark Creek Flow Zone" link, auto-updating year
...
│   ├── ForecastPanel.tsx   # QPF forecast + observed precipitation sidebar (desktop)
│   ├── ForecastDrawer.tsx  # Mobile SwipeableDrawer wrapping ForecastPanel
│   ├── LayerControl.tsx    # Tile layer toggle with tooltips (MUI ToggleButtonGroup)
│   ├── PrecipLegend.tsx    # Precipitation intensity color scale legend
│   ├── WatershedPopup.tsx  # Marker popup with rainfall/forecast rows and detail link
│   ├── WeatherSummaryBar.tsx # Aggregate rain/rising counts with freshness timestamp and refresh
...
**Footer Component** (`src/components/core/Footer.tsx`):

- Simple footer with copyright notice and app branding
- Auto-updating year via `new Date().getFullYear()`
- Clickable "Ozark Creek Flow Zone" link (placeholder `#` href) with hover effect (color changes to primary.main, underline appears)
- Theme-aware styling via `theme.palette.background.paper` and `theme.palette.divider`
- Centered layout with `Container maxWidth="sm"`
...
- **MapView.tsx**: Leaflet map container with CartoDB base tiles using three-layer tile sandwich pattern: (1) base map without labels (light: `voyager_nolabels`, dark: `dark_nolabels`), (2) IEM MRMS precipitation overlay (dynamic URL: `mesonet.agron.iastate.edu/cache/tile.py/1.0.0/{layer}/{z}/{x}/{y}.png` with layer ID interpolation at 0.35 opacity), (3) labels-only layer on top (light: `voyager_only_labels`, dark: `dark_only_labels` with zIndex 650) to ensure city names and state labels remain visible above precipitation data. CircleMarker watershed markers (SVG-based, 12px radius, 2.5px white stroke). Each marker uses `WatershedMarker` component which calls `useWatershedIntelligence(gaugeId, lat, lng)` to fetch forecast and precipitation data for popup. Uses `useGaugeDataContext()` to fetch real-time readings and `useMemo` to compute marker positions/colors on gauge data changes. Determines marker color via `getWatershedMarkerColor(statuses)` where statuses are computed per-stream using `determineLevel(reading.value, targetLevels)`. Glassmorphic popup styling with theme-aware backdrop-filter and alpha-blended backgrounds. Center: [35.5, -93.5] (Ozarks), default zoom: 8. Accepts `highlightedGauges` prop — markers not in the set render at 25% opacity for drill-down highlighting from WeatherSummaryBar clicks.
- **LayerControl.tsx**: MUI `ToggleButtonGroup` with glassmorphic background (`glassmorphism.dark` or `.light` from waterTheme). Positioned absolute (bottom-left, z-index 1000, 16px offset). Exclusive selection between radar, 24h, 48h, 72h layers. Selected button has gradient background (`linear-gradient(135deg, #30cfd0, #330867)`) with glow box-shadow. Radar button has `<Radar />` icon, 24h has `<WaterDrop />`, others text-only. All buttons have accessible aria-labels and descriptive tooltips (e.g., "Live NEXRAD radar", "Accumulated rainfall — last 24 hours") via MUI Tooltip with top placement.
- **PrecipLegend.tsx**: Precipitation intensity color scale legend. Positioned absolute (bottom-left, 64px above LayerControl, z-index 1000). Shows 7-step color gradient (green→yellow→orange→red) with rainfall labels (0.1" to 5.0+ inches). Hidden when activeLayer is 'radar'. Glassmorphic styling matches LayerControl. Horizontal layout with stacked color bars and small labels below. Title: "Rainfall (inches)".
- **WatershedPopup.tsx**: Enhanced popup content for map markers with theme-aware styling via prop-based color computation. Receives `isDark` boolean prop from MapView parent (avoids redundant `useTheme()` calls in Leaflet portal context). Computes text colors (textPrimary, textSecondary, textMuted, textDisabled) and border colors directly from prop for optimal contrast in dark/light modes. Displays gauge name (bold, 0.85rem), gradient divider line, current reading + trend label (colored via `TREND_COLORS` mapping: Rising=#4caf50, Falling=#f44336, Holding=#ff9800). Shows rainfall row (24h/48h inches with `<WaterDrop />` icon, cyan background highlight — darker opacity in dark mode for better contrast) and forecast peak row (stage + day with `<TrendingUp />` icon, green background highlight) when data available. Includes stream list with individual status chips. Stream names are react-router `<Link>` components to `/stream/:streamId` using `getStreamIdFromName()` utility. Status chips use `STATUS_HEX_COLORS` for background with glow box-shadow. Shows "Awaiting data..." (italic, disabled color) when no gauge reading. Displays loading skeletons while `intelligenceLoading` is true. Hover effect on stream rows (rgba(48, 207, 208, 0.08) background). "View Watershed Forecast →" link at bottom navigates to `/precipitation/watershed/:gaugeId`.
- **ForecastPanel.tsx**: Tabbed glassmorphic sidebar (Forecast vs Observed — renamed from "Forecast (QPF)" for clarity) with full-width primary tabs (48px height, gradient indicator). Sub-tab selectors for forecast days (Day 1-3 WPC static GIF URLs: `fill_94qwbg.gif`, `fill_98qwbg.gif`, `fill_99qwbg.gif`) or observed periods (Today/Yesterday/2 Days Ago WPC daily precipitation GIFs generated via `getObservedUrl(daysAgo)` helper using `date.setDate(date.getDate() - daysAgo)` to compute date-stamped URLs: `usa-dlyprcp-{yyyymmdd}_sm_wbg.gif`). Defaults to Yesterday tab (index 1) for Observed mode since Today images are often not yet published. Shows explicit date labels below observed images (e.g., "Wed, Mar 12") and context caption above QPF images ("Expected rainfall accumulation (inches)"). Uses internal `ImageWithFallback` component that catches `onError` events and displays contextual fallback UI with `<CloudOff />` icon, message text ("Today's observation not yet published" for Today tab, generic message for others), and gradient-styled external link ("View on source site"). Images have rounded corners, subtle border, and drop shadow. Fallback URLs: `wpc.ncep.noaa.gov/qpf/qpf2.shtml` (QPF), `wpc.ncep.noaa.gov/qpf/obsmaps/obsprecip.php` (Observed). Source attribution: "NOAA Weather Prediction Center" for both tabs.
- **ForecastDrawer.tsx**: Mobile-only bottom sheet drawer (SwipeableDrawer) wrapping ForecastPanel. Slides up from bottom with 70vh height. Includes drag handle (40px x 4px gray bar), header with cloud icon + "Forecast & Precipitation" title, and close button. Glassmorphic styling with rounded top corners (16px radius). Positioned z-index above map. Opened via floating FAB button (gradient, cyan glow) positioned bottom-right (16px offset) with cloud icon.
- **PrecipitationMap.tsx**: Main page component with responsive layout. Desktop (>= md breakpoint): flex column with WeatherSummaryBar at top, flex row below with map + 350px forecast panel. Mobile (< md): flex column with WeatherSummaryBar, map fills remaining space (flex: 1), floating FAB button to open forecast drawer. Uses `groupStreamsByWatershed(allStreams)` to prepare watershed marker data, passes to MapView. State: `activeLayer` (PrecipLayer type) for tile control, `drawerOpen` (boolean) for mobile drawer, `lastFetched` (Date | null) for freshness timestamp, `refreshKey` (number) to trigger cache-busting re-fetch, `highlightedGauges` (Set<string> | null) for drill-down highlighting. Fetches batched intelligence data (forecast + precipitation) for all watersheds on mount using staggered requests (100ms delay per gauge) to avoid API rate limits. Stores in `precipData` and `forecastData` Maps passed to WeatherSummaryBar and MapView. Refresh function (`refreshIntelligence`) clears localStorage caches and increments refreshKey to trigger useEffect re-run. Computes `rainyGaugeIds` and `risingGaugeIds` sets for highlighting. Passes callbacks to WeatherSummaryBar for clickable counts (toggles highlight state).
- **WeatherSummaryBar.tsx**: Glassmorphic bar displayed above map showing aggregate watershed weather summary. Counts watersheds with rainfall (>0.1 inch in 24h) and gauges forecast rising (stage increase >0.1 ft). Displays counts with icons (`<WaterDrop />` for rain, `<TrendingUp />` for rising). Shows "All clear — no recent rain or rising forecasts" when both counts are zero (instead of hiding). Includes relative timestamp ("Xm ago" via `getRelativeTime()` helper) showing `lastFetched` date, and refresh button (`<Refresh />` icon, disabled while loading) on right side. Counts are clickable (cursor pointer, hover background) — clicking triggers `onHighlightRainy`/`onHighlightRising` callbacks to dim non-matching markers on map. Shows loading skeleton while intelligence data fetching. Uses `glassmorphism.dark`/`.light` with bottom border.
...
**Known Edge Cases:** Tile loading failures handled gracefully by Leaflet (blank tiles). WPC image failures (QPF forecast and observed daily precipitation) trigger fallback UI with "View on source site" link. Missing gauge data shows neutral gray markers with "Loading..." in popups.
...
- `src/utils/streamLevels.ts`: Level/trend calculation logic; exports `STATUS_HEX_COLORS` (canonical hex per status), `STATUS_GRADIENT_COLORS` (primary/secondary gradient pairs), and `STATUS_LABELS` (human-readable label strings: "Too Low", "Low", "Optimal", "High") — single source of truth for status colors and labels across all components
...
- `src/components/precipitation/WeatherSummaryBar.tsx`: Aggregate weather summary bar — counts watersheds with rain (>0.1 inch 24h) and gauges forecast rising (stage increase >0.1 ft), displays with icons (`<WaterDrop />`, `<TrendingUp />`), uses glassmorphism styling, shows loading skeleton, displays "all clear" message when no activity (instead of hiding). Includes relative timestamp ("Xm ago") and refresh button. Counts are clickable to highlight relevant markers on map.
- `src/components/precipitation/ForecastDrawer.tsx`: Mobile SwipeableDrawer component wrapping ForecastPanel — slides up from bottom with 70vh height, drag handle, header with cloud icon and close button, glassmorphic styling. Opened via floating FAB positioned bottom-right on mobile.
- `src/components/precipitation/PrecipLegend.tsx`: Precipitation intensity color scale legend — 7-step gradient (0.1" to 5.0+ inches) with rainfall labels, positioned above LayerControl on map, hidden for radar layer. Glassmorphic styling matches LayerControl.
- `src/components/precipitation/LayerControl.tsx`: MUI ToggleButtonGroup for precipitation layer selection — includes descriptive tooltips ("Live NEXRAD radar", "Accumulated rainfall — last 24 hours", etc.) via MUI Tooltip with top placement and arrow styling.
...
- Security headers applied globally: `X-Frame-Options`, `X-XSS-Protection`, `X-Content-Type-Options`, `Referrer-Policy`, `Content-Security-Policy` (allowlists domains for scripts, styles, fonts, images, and API connections: basemaps.cartocdn.com, mesonet.agron.iastate.edu, wpc.ncep.noaa.gov, waterservices.usgs.gov, api.water.noaa.gov, mapservices.weather.noaa.gov, fonts.googleapis.com, fonts.gstatic.com, unpkg.com)
...
**Recent Enhancement: Three-Layer Tile Sandwich Pattern (MapView.tsx, commit 210d7ca)**

- **Tile Layer Strategy**: Implemented three-layer Leaflet tile sandwich to keep geographic labels visible above precipitation data — base map layer without labels (CartoDB `voyager_nolabels` / `dark_nolabels`), precipitation overlay in middle (IEM MRMS at 0.35 opacity), labels-only layer on top (CartoDB `voyager_only_labels` / `dark_only_labels` with zIndex 650)
- **Problem Solved**: Previous single base map + precipitation approach caused labels to be obscured by opaque tile pixels, making map navigation difficult. Three-layer approach ensures city names, state labels, and street names remain readable while precipitation data remains visible underneath
- **Layer Ordering**: Base map (z~0) → precipitation overlay (z~100, 0.35 opacity) → labels (z=650). CartoDB label layers designed to overlay transparently without blocking underlying data
- **Opacity Adjustment**: Precipitation opacity set to 0.35 (down from 0.6) to further improve readability of base map features and watershed marker visibility
- **WatershedPopup Theme Handling**: Simplified theme approach by passing `isDark` boolean prop from MapView parent instead of using `useTheme()` hook in each popup instance — eliminates redundant theme context subscriptions in Leaflet portal, improves performance, ensures consistent theme state across all markers

**Recent Security & Code Quality Improvements (commits a1c5e3f, 7e4bd3e)**

- **Turner Bend Function Hardening** (`netlify/functions/turner-bend.js`): Enhanced security and code quality — CORS headers restricted to production domain (`process.env.URL || 'https://ozarkcreekflowzone.com'`), added Authorization header support, POST `/scrape` endpoint requires Bearer token authentication via `SCRAPE_TOKEN` environment variable (returns 401 if missing/invalid), removed sensitive error messages from API responses (only returns generic error + timestamp), optimized Date handling (reuses `new Date()` instance instead of recreating for each field)
- **Import Cleanup**: Removed unused imports across components — StageChart.tsx (removed unused `alpha` from MUI/styles), WatershedPopup.tsx (removed unused `Skeleton` import that was replaced with conditional rendering)
- **Security Pattern**: Auth-protected scrape endpoint prevents unauthorized forced cache refreshes — only authenticated requests can bypass 15-minute cache timer

**Recent Feature: Precipitation Map UX Improvements (2026-03-13)**

- **Mobile Forecast Drawer**: Replaced stacked mobile forecast panel with SwipeableDrawer bottom sheet — slides up from bottom (70vh height) via floating FAB button (gradient cyan glow, bottom-right). Includes drag handle and close button. Map now fills remaining vertical space (flex: 1) instead of fixed 60vh. Improves mobile UX by maximizing map visibility while keeping forecasts accessible.
- **ForecastPanel Context**: Added plain English labels and explicit dates — renamed "Forecast (QPF)" tab to "Forecast", added "Expected rainfall accumulation (inches)" caption above QPF images, displays computed date labels (e.g., "Wed, Mar 12") below Observed images for clarity on which day is shown. Defaults to Yesterday tab for Observed mode since WPC typically publishes daily precipitation images with 1-day lag (Today images often unavailable until late evening or next day). Fallback message distinguishes between Today ("Today's observation not yet published") vs other dates (generic unavailable message).
- **Layer Control Tooltips**: Added descriptive MUI Tooltips to precipitation layer buttons — "Live NEXRAD radar", "Accumulated rainfall — last 24 hours/48 hours/72 hours". Top placement with arrow. Improves discoverability of layer meanings.
- **Precipitation Legend**: New PrecipLegend component showing 7-step color scale (green→yellow→orange→red, 0.1" to 5.0+ inches) positioned above LayerControl. Hidden for radar layer. Glassmorphic styling matches LayerControl. Helps users interpret rainfall intensity on accumulated precipitation layers.
- **Data Freshness & Refresh**: WeatherSummaryBar displays relative timestamp ("Xm ago" / "Xh ago") showing when intelligence data was last fetched, plus refresh button (IconButton with `<Refresh />` icon, disabled while loading). Refresh callback clears localStorage caches and increments `refreshKey` state to trigger useEffect re-fetch. Shows "All clear — no recent rain or rising forecasts" message when both counts are zero (instead of hiding bar entirely).
- **Clickable Summary Drill-Down**: Rain/rising counts in WeatherSummaryBar are clickable — clicking highlights relevant markers on map by dimming non-matching markers to 25% opacity. MapView accepts `highlightedGauges` Set prop. Clicking again toggles off highlight. Improves explorability by letting users quickly visually filter to active watersheds.
- **Component Files**: Added `ForecastDrawer.tsx`, `PrecipLegend.tsx`. Modified `PrecipitationMap.tsx` (mobile layout, state management, highlight logic), `ForecastPanel.tsx` (labels, dates, Yesterday default, contextual fallback), `LayerControl.tsx` (tooltips), `WeatherSummaryBar.tsx` (freshness, refresh, clickable counts), `MapView.tsx` (highlight prop, marker opacity).

**Recent Change: Footer Branding Update (2026-03-13)**

- **App Name Consistency**: Updated Footer component to display "Ozark Creek Flow Zone" instead of "Mountain Stream Tracker" — maintains consistent branding with Header logo and overall app identity
- **File Modified**: `src/components/core/Footer.tsx` (line 34, copyright link text)
