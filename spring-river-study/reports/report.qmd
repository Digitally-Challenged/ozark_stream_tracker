...
subtitle: "Base flow, floods, and precipitation over the Mammoth Spring recharge basin"
...
idx = {k: tbl(f"phase6_indices_{k}") for k in ("basin", "USC00238880", "KUNO")}
...
- **Recharge basin.** No dye-trace delineation was obtained. The basin is approximated as a 30 km buffer around West Plains, MO (36.744° N, 91.852° W); basin precipitation is the PRISM 4 km grid mean over that box. Station series: USC00238880 West Plains COOP (1981+ in this build) and KUNO West Plains ASOS (1998-04+).
...
![Annual ledger: Hardy annual peak, min7, BFI, threshold days and basin precipitation by water year. Source: USGS DV/IV 07069305, PRISM 30 km basin mean; period WY `{python} ledger_span[0]`–`{python} ledger_span[1]`; approved `{python} f"{hardy_approved:.0%}"`, provisional from `{python} qa_df.loc[0, 'provisional_from']`.](figures/annual_ledger.png){#fig-ledger}
...
- Hardy (WY 2002+): raw min7 rises (`{python} trend_str(min7['hardy'], 'log-cfs')`; Pettitt step after WY `{python} pettitt['hardy'][0]`, p = `{python} g(pettitt['hardy'][1], 2)`), but trailing-365-day precipitation carries the signal (coefficient `{python} g(ols_coef['hardy']['p_trailing_in'][0], 3)` log-cfs per inch, 95% CI `{python} g(ols_coef['hardy']['p_trailing_in'][1], 3)` to `{python} g(ols_coef['hardy']['p_trailing_in'][2], 3)`); the residual trend CI includes zero.
...
Limitation: Hardy n = `{python} min7['hardy']['n']` water years; the ONI coefficient is not resolved at either gauge; basin precipitation is a 30 km buffer, not a traced recharge polygon.
...
Thesis: basin-mean precipitation has become wetter and more intense over 1981–2025, but the Sep–Feb recharge-season total has not changed; station tests are low-power.
...
- Basin (PRISM 30 km mean, n = `{python} basin_total['n']` years): `{python} sig_count['basin']`/`{python} n_index` indices BH-significant. Annual total `{python} pm(basin_total['slope'])` in/decade (95 % CI `{python} g(basin_total['lo'], 2)` to `{python} g(basin_total['hi'], 2)`; mean `{python} f"{basin_mean_total:.1f}"` in); days ≥ 1 in `{python} pm(basin_days1['slope'])`/decade (CI `{python} g(basin_days1['lo'], 2)` to `{python} g(basin_days1['hi'], 2)`); max 1-day `{python} pm(basin_max1['slope'])` in/decade (CI `{python} g(basin_max1['lo'], 2)` to `{python} g(basin_max1['hi'], 2)`); SDII `{python} pm(basin_sdii['slope'])` in/wet-day/decade (CI `{python} g(basin_sdii['lo'], 2)` to `{python} g(basin_sdii['hi'], 2)`).
- Recharge season (Sep–Feb, `{python} f"{basin_recharge_frac:.0%}"` of the annual mean): `{python} pm(basin_recharge['slope'])` in/decade (CI `{python} g(basin_recharge['lo'], 2)` to `{python} g(basin_recharge['hi'], 2)`, n = `{python} basin_recharge['n']`). The wetting is a growing-season signal.
- Stations: USC00238880 `{python} sig_count['USC00238880']`/`{python} n_index` significant (n = `{python} coop_max3['n']`; max 3-day `{python} pm(coop_max3['slope'])` in/decade, CI `{python} g(coop_max3['lo'], 2)` to `{python} g(coop_max3['hi'], 2)`, p_BH = `{python} g(trend_row('USC00238880', 'max3_in')['p_bh'], 2)`); KUNO `{python} sig_count['KUNO']`/`{python} n_index` (SDII `{python} pm(kuno_sdii['slope'])`, CI `{python} g(kuno_sdii['lo'], 2)` to `{python} g(kuno_sdii['hi'], 2)`, n = `{python} kuno_sdii['n']`). USC00238880 fails the coverage gate in `{python} len(coop_fail_years)` years, `{python} len(coop_fail_recent)` of them in 2011–2021 (`{python} ", ".join(str(y) for y in coop_fail_recent)`), which removes most of the recent wet decade from the station test; its null is low power, not evidence against the basin trend.

![`{python} idx_cap`](figures/phase6_indices.png){#fig-indices}

Limitation: PRISM is a 4 km grid mean (smoother extremes by construction) and station gaps enter it only through interpolation; the COOP record before 1981 is not yet pulled; precipitation carries no approval flag, so the all/approved rule does not apply here.
...
hyp = pd.DataFrame([
    ("Q1", "Base flow declining secularly or tracking precipitation?", "Drought-driven, not structural",
     f"Supported. Residual (non-climatic) trend: Mammoth {resid['mammoth']['slope']:+.4f} log-cfs/yr (CI {resid['mammoth']['lo']:+.4f} to {resid['mammoth']['hi']:+.4f}, n={resid['mammoth']['n']}); "
     f"Hardy {resid['hardy']['slope']:+.4f} (CI {resid['hardy']['lo']:+.4f} to {resid['hardy']['hi']:+.4f}, n={resid['hardy']['n']}). "
     f"Hardy raw min7 rises {min7['hardy']['slope']:+.3f}/yr (CI {min7['hardy']['lo']:.3f} to {min7['hardy']['hi']:.3f}) but antecedent precip explains it (R²={ols_r2['hardy']:.2f})."),
    ...
    ("Q3", "Has rainfall intensity shifted?", "Weak/inconclusive",
     f"Basin: {sig_count['basin']}/{n_index} indices up (total {basin_total['slope']:+.1f} in/decade, CI {basin_total['lo']:.1f} to {basin_total['hi']:.1f}, n={basin_total['n']}); "
     f"recharge-season total flat ({basin_recharge['slope']:+.2f}, CI {basin_recharge['lo']:.1f} to {basin_recharge['hi']:.1f}, n={basin_recharge['n']}). Station tests low-power."),
...
- **What changed.** Basin precipitation: wetter and more intense in `{python} sig_count['basin']` of `{python} n_index` indices, concentrated in the growing season. Hardy stage at fixed discharge: `{python} f"{-10 * rating[1000]['slope']:.2f}"` ft/decade lower at 1000 cfs.
- **What did not.** Non-climatic base flow at either gauge; BFI; flood magnitude and frequency; recharge-season precipitation; the inter-arrival process of major floods; flood timing within the year and the recession constants at both gauges.
...
- Recharge-basin polygon: 30 km West Plains buffer, not a dye-traced delineation.
...
- Review overrules (docs/review_phase4-6.md): complete-WY selection uses the series end date only (no interior year-long stage gaps exist); Q4 precipitation windows have no coverage gate (PRISM basin series is complete).
...
| Module | Role |
|---|---|
| `spring_river.ingest.{usgs,acis,prism,oni,nwps}` | cached, idempotent data pulls |
