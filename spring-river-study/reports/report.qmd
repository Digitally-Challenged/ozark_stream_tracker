...
Thesis: basin-mean precipitation has become more intense over 1981–2025 but not detectably wetter — the annual total carries no significant trend and the Sep–Feb recharge-season total is flat to negative. The gap-ridden single-station tests are low-power, but the `{python} wp_total['n']`-year West Plains 1948– record is not: it finds more wet days (days ≥ 1 in `{python} pm(wp_days1['slope'])`/decade, CI `{python} g(wp_days1['lo'], 2)` to `{python} g(wp_days1['hi'], 2)`) and no intensification, so the basin series' intensity signal is not corroborated at the gauge over the longer window.
...
hyp = pd.DataFrame([
    ...
    ("Q3", "Has rainfall intensity shifted?", "Weak/inconclusive",
     ...
     f"recharge-season total flat to negative ({basin_recharge['slope']:+.2f}, CI {basin_recharge['lo']:.1f} to {basin_recharge['hi']:.1f}, n={basin_recharge['n']}). "
     f"West Plains 1948– ({wp_total['n']} yr, COOP+KUNO): only days ≥ 1 in significant "
     f"({wp_days1['slope']:+.2f}/decade, CI {wp_days1['lo']:.2f} to {wp_days1['hi']:.2f}); "
     f"no intensification at the gauge. Other station tests low-power."),
