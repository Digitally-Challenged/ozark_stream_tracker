...
COMPOSITE = "West Plains composite"
idx = {k: tbl(f"phase6_indices_{k}") for k in ("basin", "USC00238880", "KUNO", "USC00230127")}
idx[COMPOSITE] = tbl("phase6_indices_westplains_composite")
...
comp_total = idx_trend(COMPOSITE, "total_in")
comp_recharge = idx_trend(COMPOSITE, "recharge_in")
comp_max1 = idx_trend(COMPOSITE, "max1_in")
comp_sdii = idx_trend(COMPOSITE, "sdii_in")
comp_days1 = idx_trend(COMPOSITE, "days_ge_1")
comp_sig = it.loc[(it["series"] == COMPOSITE) & it["significant_bh"], "index"].tolist()
...
comp_years = years_passing(idx[COMPOSITE])
comp_first_year = int(idx[COMPOSITE].loc[idx[COMPOSITE]["coverage"] >= 0.9, "year"].min())
comp_ratio = doc_re(P6, r"KUNO × (\S+) from 1998-04-01")
comp_kuno_days = doc_re(P6, r"; (\d+) KUNO days", int)
comp_coop_fill = doc_re(P6, r"KUNO days, (\d+) COOP-fallback days", int)
...
Thesis: basin-mean precipitation has become more intense over 1981–2025 but not detectably wetter — the annual total carries no significant trend and the Sep–Feb recharge-season total is flat to negative. The gap-ridden single-station tests are low-power, but the `{python} comp_total['n']`-year West Plains composite is not: it finds more wet days (days ≥ 1 in `{python} pm(comp_days1['slope'])`/decade, CI `{python} g(comp_days1['lo'], 2)` to `{python} g(comp_days1['hi'], 2)`) and no intensification, so the basin series' intensity signal is not corroborated at the gauge over the longer window.
...
- **West Plains composite** (`{python} comp_first_year`–2025, n = `{python} comp_total['n']` years): the COOP record with its missing volunteer readings replaced by the co-located KUNO ASOS (~2 mi away) scaled by the COOP/KUNO catch ratio `{python} comp_ratio` — a substituted measurement, never an interpolated one (`{python} f"{comp_kuno_days:,}"` KUNO days and `{python} comp_coop_fill` COOP-fallback days after 1998-04-01; a day missing at both gauges stays NaN). `{python} comp_years` years pass the coverage gate against `{python} years_passing(idx['USC00238880'])` for COOP alone, making this the longest and highest-power station test in the study. `{python} sig_count[COMPOSITE]`/`{python} n_index` indices are BH-significant (`{python} ", ".join(INDEX_LABELS[i] for i in comp_sig)`). The annual total rises `{python} pm(comp_total['slope'])` in/decade (95 % CI `{python} g(comp_total['lo'], 2)` to `{python} g(comp_total['hi'], 2)`, n = `{python} comp_total['n']`) but does not survive BH correction; the recharge-season total is `{python} pm(comp_recharge['slope'])` in/decade (CI `{python} g(comp_recharge['lo'], 2)` to `{python} g(comp_recharge['hi'], 2)`, n = `{python} comp_recharge['n']`). Unlike the basin series, the composite shows no intensification: max 1-day `{python} pm(comp_max1['slope'])` in/decade (CI `{python} g(comp_max1['lo'], 2)` to `{python} g(comp_max1['hi'], 2)`) and SDII `{python} pm(comp_sdii['slope'])` in/wet-day/decade (CI `{python} g(comp_sdii['lo'], 2)` to `{python} g(comp_sdii['hi'], 2)`) both have CIs spanning zero over `{python} comp_total['n']` years. What it does show is more wet days: days ≥ 1 in `{python} pm(comp_days1['slope'])`/decade (CI `{python} g(comp_days1['lo'], 2)` to `{python} g(comp_days1['hi'], 2)`), the one index that passes.
...
hyp = pd.DataFrame([
    ...
    ("Q3", "Has rainfall intensity shifted?", "Weak/inconclusive",
     ...
     f"West Plains composite ({comp_total['n']} yr, COOP+KUNO): only days ≥ 1 in significant "
     f"({comp_days1['slope']:+.2f}/decade, CI {comp_days1['lo']:.2f} to {comp_days1['hi']:.2f}); "
...
- COOP USC00238880 has `{python} coop_gaps` gaps > 7 days and fails coverage in `{python} len(coop_fail_years)` index years; its 1948–1980 record is now pulled but feeds only the West Plains composite, not the 1981+ COOP series the other analyses use. KUNO begins 1998-04.
