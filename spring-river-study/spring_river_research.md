...
| Q3 | Weak/inconclusive | **(2nd ed.)** More intense but not detectably wetter. Basin (AORC, MoDNR polygon): 6/10 indices BH-significant — max 1-day +0.26 in/decade (CI 0.05–0.50), SDII +0.032 in/wet-day/decade (CI 0.020–0.046), days ≥ 1 in +1.38/decade (CI 0.49–2.22) — but annual total is **not** significant (+0.96 in/decade, CI −1.25 to 3.12, n=45); the first edition's significant +2.4 was a property of the oversized 30 km buffer. Recharge-season total flat-to-negative (−0.81, CI −1.99 to 0.60, n=44). Single-station tests low-power (COOP coverage gaps), but the 76-year West Plains composite (COOP backfilled with KUNO × 1.068) is not: only days ≥ 1 in is BH-significant (+0.71/decade, CI 0.27 to 1.18, n=76), with max 1-day and SDII CIs spanning zero — the gauge shows more wet days, not harder rain, and does not corroborate the basin intensification over 1948–2025. |
...
### West Plains composite (Task 10)

The West Plains COOP gauge (`USC00238880`, volunteer-read, 1948-07-01→) has ~675 missing days
scattered through 2011–2021, so twelve calendar years fail the 90 % coverage gate and the recent
wet decade drops out of its trend test. The West Plains Municipal Airport ASOS (`KUNO`, ~2 mi away,
1998-04-01→) is essentially complete. Monthly totals at the two gauges agree at r = 0.86, and over
months with ≥ 25 days at both the **COOP/KUNO catch ratio is 1.068** — the ASOS undercatches
slightly, as ASOS gauges generally do.

`climate/composite.py` splices them: COOP as measured through 1998-03-31, then KUNO × 1.068 for
every day KUNO reported (10,331 days), unscaled COOP where KUNO was missing and COOP was not
(36 days), and NaN where neither reported. It **substitutes a co-located measurement; it never
interpolates** — a day missing at both gauges stays NaN, and its calendar year is still judged on
the same 90 % coverage gate. The 1948 COOP pull uses its own cache (`acis_pcpn_USC00238880_1948`,
via the new `cache_suffix` argument), so the 1981+ COOP series the ledger and other phases read is
untouched.
...
|:------|-------------:|:--------|--:|:---------------|
| total_in | +1.32 in | 0.308 to 2.35 | 76 | no (p_BH 0.055) |
| recharge_in | +0.569 in | −0.062 to 1.08 | 74 | no |
| growing_in | +0.624 in | −0.034 to 1.35 | 76 | no |
| days_ge_0p5 | +0.833 | −0.000 to 1.56 | 76 | no |
| days_ge_1 | +0.714 | 0.267 to 1.18 | 76 | **yes** (p_BH 0.014) |
...
correction across the ten indices. So the composite corrects the "station tests are low-power"
caveat rather than confirming the basin thesis: with the power problem removed, the gauge does not
