...
**P1 - Improvements:** 4. [ ] Add graceful error handling - show "unavailable" instead of error 5. [ ] Fix date parsing - use explicit format instead of `new Date(ambiguous)` 6. [ ] Add scrape locking - prevent concurrent writes to data file 7. [ ] Delete `turnerBendScraperFixed.js` - duplicate dead code
...
**P1 - UX Polish:** 4. [ ] Show "Holding" trend in StreamCard (currently hidden when stable) 5. [ ] Fix tooltip text - "rise" → "Rising", "fall" → "Falling" 6. [ ] Reduce MIN_TREND_INTERVAL from 30 to 5-10 minutes

**P2 - Data Quality:** 7. [ ] Move history from localStorage to GaugeDataContext 8. [ ] Add error handling for localStorage failures 9. [ ] Improve reading lookup (avoid timestamp matching edge cases)
